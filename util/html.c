// FILE: html.c
//
// This is a utility for CS23 search engine project ( or general case)
//
...
// Takes a string html_doc, an output string resulting_word, and a starting
// position current position and stores the next non-tag word into resulting_word.
// Returns the position where it finished parsing, or -1 if it's done.
int parseHTML(char *html_doc, char *resulting_word, int current_position) 
{
  	char c;
  	char *p1;  //!< pointer pointed to the start of a new word
  	char *p2;  //!< pointer pointed to the end of a new word

	int tag_flag = 0;
	int word_flag = 0;

	int ignore_first_two_lines = 1; // specific to the indexer because the crawler files have 2 lines of non-HTML
	int newline_count = 0;

	if(current_position == 0)
		ignore_first_two_lines = 0;

	while((c = html_doc[current_position]) != 0)
	{
		if(c == '\n')
		{
			newline_count++;
		...
		if(ignore_first_two_lines)
		{
			...
			if(word_flag)	//if you're in a word
			{
				if(!isalpha(c))
				{
					p2 = &(html_doc[current_position]);
					strncpy(resulting_word, p1, (p2-p1));
					return current_position;
				}
			}
			else		//if you're not in a word
			{
				if(isalpha(c))	// if you encounter a word
				{
					word_flag = 1;
					p1 = &(html_doc[current_position]);
				}
			}
		}
...
// takes a char array string, and a int position
// starts at position in string and finds the first word
// stores that word into word
// returns the position at the end
// returns -1 if no words left
int getNextWord(char *string, char *word, int current_position)
{
	char c;
  	char *p1;  //!< pointer pointed to the start of a new word
  	char *p2;  //!< pointer pointed to the end of a new word
	...
	while((c = string[current_position]) != 0)
	{	
		if(word_flag)	//if you're in a word
		{
			if(!isalpha(c))
			{
				p2 = &(string[current_position]);
